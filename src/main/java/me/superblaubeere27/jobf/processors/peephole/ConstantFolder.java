...
package me.superblaubeere27.jobf.processors.peephole;

import me.superblaubeere27.IPreClassProcessor;
import me.superblaubeere27.analyzer.AnalyzerResult;
import me.superblaubeere27.analyzer.MethodAnalyzer;
import me.superblaubeere27.analyzer.frame.*;
...
public class ConstantFolder implements IPreClassProcessor {
    private static final String PROCESSOR_NAME = "PeepholeOptimizer";
    private EnabledValue enabledValue = new EnabledValue(PROCESSOR_NAME, DeprecationLevel.GOOD, false);
    private BooleanValue experimantalPopFolding = new BooleanValue(PROCESSOR_NAME, "Experimental Pop folding", DeprecationLevel.OK, false);

    public void process(Collection<ClassNode> classNodes) {
            ...
            do {
                ...
                for (int i = 0; i < methodNode.instructions.size(); i++) {
                    ...
                    switch (ain.getOpcode()) {
                                ...
                                while (itr.hasNext()) {
                                    ...
                                    for (LabelNode label : ((TableSwitchInsnNode) ain).labels)
                                        if (label.getNext() != null && label.getNext().equals(ldcNode)) {
                                            results.remove(NodeUtils.getIntValue(ldcNode));
                                            itr.remove();
                                        }
                                }
                            }
                        ...
                        case POP:
                        case POP2: {
                            if (!experimantalPopFolding.getObject()) {
                                break;
                            }
                            ...
                            for (Frame frame0 : frames) {
                                ...
                                if (frame.getRemoved().get(0) instanceof LdcFrame && (ain.getOpcode() != POP2 || frame.getRemoved().size() == 2 && frame.getRemoved().get(1) instanceof LdcFrame)) {
