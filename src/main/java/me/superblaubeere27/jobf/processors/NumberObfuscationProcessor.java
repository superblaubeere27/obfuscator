...
public class NumberObfuscationProcessor implements IClassProcessor {
    ...
    private boolean lenghtMode = true;
    private boolean xorMode = true;
    private boolean simpleMathMode = true;
...
//                        if (method.maxStack < 4) {
                        method.maxStack += 4;
//                        }
                        method.instructions.insertBefore(abstractInsnNode, getInstructions(number));
                        method.instructions.remove(abstractInsnNode);
                    }
                }
            }
...
//            clInit.maxStack = Math.max(clInit.maxStack, 6);
    ...
    private InsnList getInstructions(int value) {
        /*
